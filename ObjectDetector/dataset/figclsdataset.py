...
class FigClsDataset(Dataset):
    ...
    def __init__(self, model_type, data_dir='training_data/JPEGImages', anno_dir='training_data/annotation/',
        ...
        self.class_ids = np.arange(1,90) #scale bar and subfigure label
...
#         class_labels = ["a","b","c","d","e","f"]
        for bbox in bboxes:
            if bbox["bbox"][2] > self.min_size and bbox["bbox"][3] > self.min_size:
                labels.append([])
                if bbox["category"] == "image":
                    class_label = 0
                elif bbox["category"] == "nonimage":
                    class_label = 1
                elif bbox["category"] == "subfigure":
                    class_label = 2
                elif bbox["category"] == "scalebar":
                    class_label = 3
                else:
                    raise ValueError("unrecognized category name(%s)"%bbox["category"])
#                 if "sub" in bbox["category"]:
#                     class_label = 0#class_labels.index(bbox["category"].split('_')[-1])
