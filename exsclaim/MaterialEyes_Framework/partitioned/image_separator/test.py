...
def main():
    ...
    parser.add_argument('--bbox_expand', type=float, default=0)
    ...
    for sample_img in img_names:
        ...
        for i in range(len(bboxes)):
            ...
            if args.bbox_expand:
                ...
                x_c = (x1+x0)/2
                y_c = (y1+y0)/2
                w_c_new = w_c*(1+args.bbox_expand)
                h_c_new = h_c*(1+args.bbox_expand)
                x0 = x_c - h_c_new/2
                x1 = x_c + h_c_new/2
                y0 = y_c - w_c_new/2
                y1 = y_c + w_c_new/2
...
#             print(x0,y0,x1,y1,type(x0))
            ...
            tmp_boxes[i+1] = "x0={},y0={},x1={},y1={}".format(x0,y0,x1,y1)
...
#             os.makedirs(os.path.join(args.result_dir,sample_img.split("/")[-1].split(".")[0]))
        if args.save_bbox:
            saved_bboxes[sample_img] = tmp_boxes
        if args.save_image:
            for i in range(len(sub_figures)):
                ...
                print("save cropped sub-figure at {}".format(os.path.join(args.result_dir,sample_img.split("/")[-1].split(".")[0]+"-{}_output.png".format(i+1))))
#             sub_figures[i].save(os.path.join(os.path.join(args.result_dir,sample_img.split("/")[-1].split(".")[0]),"{}_output.png".format(i+1)))

    if args.save_bbox:
        with open(os.path.join(args.result_dir,"bboxes.yaml"),"w") as outfile:
            yaml.dump(saved_bboxes, outfile, default_flow_style=False)
        print("bbox location has been saved at {}".format(os.path.join(args.result_dir,"bboxes.yaml")))
