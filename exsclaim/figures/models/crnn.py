...
class CRNN(nn.Module):
    def __init__(
        ...
        if configuration is not None:
            ...
            convolution_layers = config.get("convolution_layers", convolution_layers)
            ...
            batch_normalization = config.get("batch_normalization", batch_normalization)
            dropout = config.get("dropout", dropout)
            activation_type = config.get("activation_type", activation_type)
            recurrent_layers = config.get("recurrent_layers", recurrent_layers)
        ...
        if in_channels is None:
            if convolution_layers < 8:
                ...
                in_channels = [
                    min(2 ** (i // 3 + 6), 2014) for i in range(convolution_layers)
                ]
        ...
        if max_pooling is None:
            max_pooling = list(np.linspace(0, convolution_layers - 1, 3).astype(int))
        ...
        for layer in range(convolution_layers):
            ...
            convolution_layer = nn.Conv2d(
                in_channel, out_channel, cnn_kernel_size, padding=padding
            )
            cnn.add_module("Convolution {}".format(layer), convolution_layer)
            if layer in max_pooling:
                cnn.add_module(
                    "Max Pooling {}".format(layer),
                    nn.MaxPool2d(
                        max_pooling_kernel, max_pooling_stride, max_pooling_padding
                    ),
                )
