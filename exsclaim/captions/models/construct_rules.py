...
def caption_parse_rules(offset, position_key, separate, char_type, char_num):
    """Create matching rules and based on custom characteristics of caption text.
    ...
    :param char_type: the character type (letter-> alpha or ALPHA (capitalized)
        digit-> number ... etc)
    :param char_num: number of consecutive characters between delimeters

    :return: A spaCy Match pattern. A pattern consists of a list of dicts, where
    ...
    # Get subfigure label offset char (i.e parenthesis mark off test specific
    ...
    # Get subfigure label separation char (i.e this punctuation separated
    # consective chars)
    separate_choices = {
        "comma": ",",
        "dash": "–",
    ...
    # Get character type (input to SHAPE), or list of characters from a
    # predefined vocabulary
    character_choices = {
        "alpha": "x",
        "ALPHA": "X",
        "digit": "d",
        "roman": roman_keywords,
    ...
    if not keywords:
        if separate_delimiter not in ["none", "and"]:
            body = [{"SHAPE": character_delimiter}, {"ORTH": separate_delimiter}] * (
                char_num - 2
            ) + [
                {"SHAPE": character_delimiter},
                {"ORTH": separate_delimiter, "OP": "?"},
                {"ORTH": "and", "OP": "?"},
            ] * 2
        else:
            body = [{"SHAPE": character_delimiter}, {"ORTH": separate_delimiter}] * (
        ...
        if separate_delimiter not in ["none", "and"]:
            body = [{"LOWER": {"IN": keywords}}, {"ORTH": separate_delimiter}] * (
                char_num - 2
            ) + [
                {"LOWER": {"IN": keywords}},
                {"ORTH": separate_delimiter, "OP": "?"},
    ...
    if position_key == 0:
        ...
        return (
            [{"ORTH": offset_delimiter[0]}]
            + body[:-1]
            + [{"ORTH": offset_delimiter[1]}]
        )
    else:
        raise ValueError(
            "The offset delimiter must appear to the left (0), right (1),"
            "or both sides (2) of the character!"
        )


def caption_pattern_collection(
    offsets, position_keys, separations, char_types, char_nums
):
    """Wrapper for caption_parse_rules to create collection of rules and descriptions

    :return: List of tuples representing the name of each token (rule) with a list of
        dicts, where each dict describes a token.
    """
    full = []
...
def caption_nlp_model():
    ...
    :param: offsets: punctuation used to set off characters that are explanatory
        (i.e. a parenthesis)
    ...
    :param: char_types: the character type (letter-> alpha or ALPHA (capitalized),
        digit-> number ... etc)
    :param: char_nums: number of consecutive characters between delimeters
    :param: custom_pattern: a list of tuples containing any custom patterns
        (from observation)
    ...
    custom_patterns = [
        {
            "label": "parenthesis_02_none_ALPHA_02",
            "pattern": [
                {"ORTH": "("},
                {"TEXT": {"REGEX": "[A-Z]{1}\\d{1}"}},
                {"ORTH": ")"},
            ],
        },
        {
        ...
        {
            "label": "period_00_none_alpha_01",
            "pattern": [{"ORTH": "."}, {"SHAPE": "x"}],
        },
        {
            "label": "period_00_comma_alpha_02",
            "pattern": [{"ORTH": "."}, {"SHAPE": "x"}, {"ORTH": ","}, {"SHAPE": "x"}],
        },
        {
            "label": "period_00_dash_alpha_02",
            "pattern": [{"ORTH": "."}, {"SHAPE": "x"}, {"ORTH": "–"}, {"SHAPE": "x"}],
        },
    ]
    ...
    caption_patterns = caption_pattern_collection(
        offsets, position_keys, separations, char_types, char_nums
    )
