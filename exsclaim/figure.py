...
class FigureSeparator(ExsclaimTool):
    ...
    def _load_model(self):
        ...
        scale_label_recognition_checkpoint = model_path + "checkpoints/scale_label_recognition_model.pt"
        # load an object detection model pre-trained on COCO
        scale_label_recognition_model = CRNN(3, 22)
        if self.cuda:
            scale_label_recognition_model.load_state_dict(torch.load(scale_label_recognition_checkpoint)["model_state_dict"])
            scale_label_recognition_model = scale_label_recognition_model.cuda()
        else:
            scale_label_recognition_model.load_state_dict(torch.load(scale_label_recognition_checkpoint, map_location='cpu')["model_state_dict"])
        self.scale_label_recognition_model = scale_label_recognition_model
