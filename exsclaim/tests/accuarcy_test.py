...
class TestScaleDetection(unittest.TestCase):
    ...
    def test_scale_object_detection_accuracy(self):
        ...
        _ = evaluate(
            self.figure_separator.scale_bar_detection_model,
            dataloader,
            self.figure_separator.device,
        )
    ...
    def test_scale_label_reading_accuracy(self):
        ...
        # set constants
        scale_label_data = (
            pathlib.Path(__file__).parent / "data" / "scale_label_dataset"
        )
        ...
        while (
            high_total < min_high_test_images or low_total < min_low_test_images
        ) and total < max_test_images:
            total += 1
            # randomly pick label
        ...
        self.assertGreater(
            low_accuracy,
            expected_low_confidence_correct,
            (
        ...
        self.assertGreater(
            high_accuracy,
            expected_high_confidence_correct,
            (
                "Scale label reading had poor accuracy with {} % correct, "
                "less than desired {}% for predictions with confidence score "
                "of {}".format(
